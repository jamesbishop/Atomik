...
- (void)setRetinaScale:(float)newScale {
    if(isRetina) {
        scale = (newScale / [_sceneManager graphicsScale]);
    } else {
        self.scale = newScale;
    }
}
...
// New method to handle image and texture scaling of retina images for OpenGL.
- (id)initWithImage:(NSString*)aImage width:(int)width height:(int)height {
	self = [super init];
	if (self != nil) {
        filter = GL_NEAREST;
		scale = 1.0f;
		imageName = [NSString stringWithFormat:@"%@_(%dx%d).png", aImage, width, height];
        _retinaWidth = width;
        _retinaHeight = height;
...
- (void)initImplementation {
    ...
    texture = [[_textureManager getTextureWithName:imageName] retain];
	...
	maxTexWidth = (float)imageWidth / (float)textureWidth;
	maxTexHeight = (float)imageHeight / (float)textureHeight;
    ...
    if(USE_RETINA_DISPLAY) {
        isRetina = NO;
        if((_retinaWidth > 0) && (_retinaHeight > 0)) {
            if((imageWidth > _retinaWidth) && (imageHeight > _retinaHeight)) {
                isRetina = YES;
                [self setRetinaScale:scale];
            } else {
                /*
                NSLog(@" -> NON-RETINA IMAGE DETECTED [%@] (%d = %d) ... (%d = %d)!", imageName, imageWidth, _retinaWidth, imageHeight, _retinaHeight);
                
                NSLog(@"Image Data: [%@], W=%d, H=%d, TW=%d, TH=%d, TWR=%.2f, THR=%.2f", imageName, imageWidth, imageHeight,
                 textureWidth, textureHeight, texWidthRatio, texHeightRatio);
                */
...
- (NSString *)description {
	return [NSString stringWithFormat:@"texture:%d width:%d height:%d texWidth:%d texHeight:%d maxTexWidth:%.3f maxTexHeight:%.3f angle:%f scale:%f", 
...
- (Image*)getSubImageAtPoint:(CGPoint)aPoint subImageWidth:(GLuint)aImageWidth subImageHeight:(GLuint)aImageHeight scale:(float)aScale {
    ...
    // We mark the subimage to be returned as autorelease as its the 
    // resposbility of the caller to retain the returned subimage.
...
- (void)renderAt:(CGPoint)point texCoords:(Quad2f*)tc quadVertices:(Quad2f*)qv {
	...
	glColor4f(_colorfilter.red, _colorfilter.green, _colorfilter.blue, _colorfilter.alpha * [_sceneManager globalAlpha]);
	...
	if([texture name] != [_sceneManager currentlyBoundTexture]) {
		[_sceneManager setCurrentlyBoundTexture:[texture name]];
