...
- (id) initWithData:(const void*)data pixelFormat:(Texture2DPixelFormat)pixelFormat pixelsWide:(NSUInteger)width pixelsHigh:(NSUInteger)height 
    contentSize:(CGSize)size filter:(GLenum)filter {
...
{
	return [NSString stringWithFormat:@"<%@ = | Name = %i | Dimensions = %ix%i | Coordinates = (%.2f, %.2f)>", [self class], _name, _width, _height, _maxS, _maxT];
}
...
- (id) initWithImage:(UIImage*)uiImage filter:(GLenum)filter {
	...
	if(image == NULL) {
#ifdef DEBUG_MODE
		NSLog(@"ERROR - Texture2D: Image is Null");
#endif
		[self release];
		return nil;
	}
